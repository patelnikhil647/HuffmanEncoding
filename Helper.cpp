...
int FancyInputStream::read_shorter_int() {
    if(buffer_index != 8) {
        error("Attempting to read int when bitwise buffer is not empty");
    }
    unsigned int num;                             // temporary variable to store the number
    input_file.read((char*)&num, sizeof(char)*3); // read the number and store it in 'num'
    if(input_file.eof()) {                        // not enough bytes in the file to read it
        error("Not enough bytes to read the next int");
    }
    return num;
...
void FancyOutputStream::write_shorter_int(int const & num) {
    if(buffer_index != 0) {
        error("Attempting to write int when bitwise buffer is not empty");
    }
    output_file.write((char*)&num, sizeof(char)*3); // write 'num' to file with size of 3 bytes (instead of 4)
}
